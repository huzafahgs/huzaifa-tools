...
- The Vercel Node function verifies the bearer token with Supabase Auth, requires a confirmed non-anonymous user, reserves database quota, then calls Google's fixed native GenerateContent endpoint for `gemini-3.5-flash-lite`.
...
Official references: [Google compatibility](https://ai.google.dev/gemini-api/docs/openai), [model](https://ai.google.dev/gemini-api/docs/models/gemini-3.5-flash-lite), [pricing](https://ai.google.dev/gemini-api/docs/pricing), [data terms](https://ai.google.dev/gemini-api/terms).
