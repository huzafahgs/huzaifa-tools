...
- The Vercel Node function verifies the bearer token with Supabase Auth, requires a confirmed non-anonymous user, reserves database quota, then calls Google's fixed native GenerateContent endpoint for `gemini-2.5-flash-lite`.
...
Exact model: **gemini-2.5-flash-lite**. Thinking is disabled with `thinkingBudget: 0`; output is capped with `maxOutputTokens: 2200`. Accept only one model text candidate ending with `STOP`; reject blocked, incomplete, function-call, executable-code, empty or oversized results. No search or code execution is enabled.
