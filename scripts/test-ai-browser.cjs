...
(async () => {
 ...
 for (const width of viewports) {
  ...
  await page.route('**/api/ai',async route=>{
   ...
   const handler=createHandler({env:{NODE_ENV:'test',GEMINI_API_KEY:mode==='unavailable'?'':'fixture-only',VITE_SUPABASE_URL:'https://test.supabase.co',VITE_SUPABASE_PUBLISHABLE_KEY:'sb_publishable_fixture'},request:async url=>{
