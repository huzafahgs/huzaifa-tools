...
(async () => {
 ...
 for (const width of [1440,375,320]) {
  ...
  let mode='success',calls=0, cloudWrites=[],favorites=[],history=[];
  ...
  await page.route('**/api/ai',async route=>{
   ...
   if(request.method()==='POST'){calls++;if(mode==='slow')await new Promise(r=>setTimeout(r,800));}
   const handler=createHandler({env:{NODE_ENV:'test',OPENAI_API_KEY:mode==='unavailable'?'':'fixture-only',VITE_SUPABASE_URL:'https://test.supabase.co',VITE_SUPABASE_PUBLISHABLE_KEY:'sb_publishable_fixture'},request:async url=>{
    if(url.includes('ai_tools_ready'))return{ok:true,json:async()=>true};
  ...
  for(const tool of tools){
   ...
   const button=page.getByRole('button',{name:tool.action,exact:true});await button.click();await page.getByRole('alert').filter({hasText:'20–12,000'}).waitFor();
  ...
  await page.goto(base+'/all-tools?category=AI');assert.equal(await page.locator('.tool-card-link').count(),5);checks++;
