...
const bodyFor = t => ({ tool: t.slug, text: t.example, options: Object.fromEntries(t.fields.map(f => [f.key, f.values[0]])) });
...
async function run(req = reqFor(), responses = [reply(200, { id: 'verified', email_confirmed_at: '2026-01-01' }), reply(200, true), reply(200, success)], override = env) {
  const calls = [];
...
test('five allowlisted tools validate all supported options and use bounded non-stored provider requests', () => {
  for (const tool of tools) {
    for (const field of tool.fields) for (const value of field.values) {
      ...
      assert.equal(request.model, 'gpt-4.1-mini'); assert.equal(request.input[0].content[0].text, tool.example);
    }
  }
});
...
test('no configuration and missing migration fail closed', async () => {
  ...
  const ready = await run({ method: 'GET', headers: {} }, [reply(200, true)]); assert.equal(ready.body.configured, true); assert.equal(ready.body.catalogReady, true);
  const noKey = await run({ method: 'GET', headers: {} }, [reply(200, true)], { ...env, OPENAI_API_KEY: '' }); assert.equal(noKey.body.configured, false); assert.equal(noKey.body.catalogReady, true);
});
test('verified confirmed user and atomic quota precede fixed provider request', async () => {
  for (const tool of tools) {
    ...
    assert.ok(out.calls[0].url.endsWith('/auth/v1/user')); assert.ok(out.calls[1].url.endsWith('/consume_ai_quota'));
    assert.equal(out.calls[2].url, 'https://api.openai.com/v1/responses');
    assert.equal(out.calls[1].options.body, '{}'); assert.equal(out.headers['Cache-Control'], 'no-store');
